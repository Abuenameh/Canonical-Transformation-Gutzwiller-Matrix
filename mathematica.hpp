...
ostream& operator<<(ostream& out, const mathematica<Matrix<Scalar, Dynamic, Dynamic> >& m) {
    Matrix<Scalar, Dynamic, Dynamic>& mat = m.v;
    int r = mat.rows();
    int c = mat.cols();
...
ostream& operator<<(ostream& out, const mathematica<std::array<T, N> >& arr) {
    std::array<T, N>& a = arr.v;
